...
numClasses = 16; % for the indian pines imagenumPix = 5;
    for j = 1: length(lambda)
        ...
        for i = 1: size(trainData2,3) % for each point in the training data, apply low group rank sparse coding
            ...
            alphasP_train = [alphasP_train abunFromCenPix]; % alphasP_train = features x pixels
            i
        end
        ...
        %alphasP_train as input should be: alphasP_train' = pixels x features
        ...
        %maxIter = 1000; stepLen = 0.1; 
        %[ theBeta ] = multinomialLogisticRegressionL1_n( alphasP_train, trainLabels, lambda_EN, alpha_EN,maxIter, stepLen,0);
        %[ovAcc,logLik] = evaluateLogisticRegressionModel_n( alphasP_test', testLabels, theBeta);
        
        %trainLabels_int = int32(trainLabels); 
        %[ theBeta ] = mlr_ADMM( alphasP_train, trainLabels, alphasP_test', testLabels, lambda_EN, alpha_EN, 1 , trainLabels_int);
    end
end
